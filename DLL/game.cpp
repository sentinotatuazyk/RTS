...
Game::Game() :
	m_window(sf::VideoMode({ 1280, 720 }), "RTS Game/Konrad Toborek", sf::Style::Titlebar | sf::Style::Close),
	m_menu(m_window),
...
void Game::update(float deltaTime) {
	...
	if (m_state == State::Menu) {
		...
		if (m_isPrepPhase) {
			...
			if (m_prepTimer <= 0.f) {
				if (!m_player.hasTownHall()) {
					m_gameOver = true;
					m_gameOverReason = "Time is up! You didn't build a Town Hall!";
				}
				else {
					m_isPrepPhase = false;
					m_waveTimer = 5.f; 
...
void Game::render() {
	...
	if (m_state == State::Menu) {
		...
		m_window.setView(m_window.getDefaultView());
		m_ui.draw(m_window,m_player, m_view, m_prepTimer, m_waveTimer, m_currentWave, m_isPrepPhase);
...
void Game::spawnWave(int waveNumber) {
	...
	bool townHallExists = false;
	for (const auto& b : m_player.getBuildings()) {
		if (b->getType() == BuildingType::TownHall) {
			targetPos = b->getPosition();
			townHallExists = true;
			break;
		}
	}
	
	int goblins = 2 + (waveNumber * 2);
	int orcs = waveNumber / 2;
	int trolls = waveNumber / 5;

	auto spawnEnemyType = [&](EnemyType type, int count) {
		for (int i = 0; i < count; ++i) {
			...
			if (edge == 0) spawnPos = { static_cast<float>(rand() % static_cast<int>(mapW)), 0.f };
			else if (edge == 1) spawnPos = { mapW, static_cast<float>(rand() % static_cast<int>(mapH)) };
			else if (edge == 2) spawnPos = { static_cast<float>(rand() % static_cast<int>(mapW)), mapH };
			else spawnPos = { 0.f, static_cast<float>(rand() % static_cast<int>(mapH)) };
			addEnemy(findValidSpawnPosition(spawnPos.x, spawnPos.y), type);
			if (!townHallExists) {
				float dist = 1000.f;
				for (const auto& u : m_player.getUnits()) {
					sf::Vector2f dir = u.getPosition() - targetPos;
					dist = std::sqrt(dir.x * dir.x + dir.y * dir.y);
					if (dist < std::sqrt(dir.x * dir.x + dir.y * dir.y)) {
						dist = std::sqrt(dir.x * dir.x + dir.y * dir.y);
						targetPos = u.getPosition();
					}
				}
			}
			m_enemies.back().moveTo(targetPos, m_map);
			m_enemies.back().setState(EnemyState::Aggressive);
		}
	};

	spawnEnemyType(EnemyType::Goblin, goblins);
	spawnEnemyType(EnemyType::Orc, orcs);
	spawnEnemyType(EnemyType::Troll, trolls);
}
