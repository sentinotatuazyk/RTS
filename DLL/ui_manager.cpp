...
#include "map.h"
#include "enemy.h"
#include "utils.h"  // toString(UnitType)
#include "buildings.h"
...
void UIManager::drawTimerUI(sf::RenderWindow& window, float prepTimer, float waveTimer, int currWave, bool isPrepPhase) {
    std::ostringstream ss;
	float activeTimer = isPrepPhase ? prepTimer : waveTimer;
    int activeTimerInt = std::max(0, static_cast<int>(activeTimer));

	int minutes = activeTimerInt / 60;
	int seconds = activeTimerInt % 60;
    if (isPrepPhase) {
        ss << "Preparation Phase\nTime left: ";
	} 
    else {
        ss << "Wave " << currWave << "\nTime left: ";
    }
	...
	sf::Text t(m_font, ss.str(), 24);
    t.setFillColor(isPrepPhase ? sf::Color::Green : sf::Color::Red);


	auto bounds = t.getLocalBounds();
    t.setOrigin({ bounds.position.x + bounds.size.x / 2.f, bounds.position.y });

	float x = window.getSize().x / 2.f;
	float y = 15.f;
	t.setPosition({ x, y });

	sf::RectangleShape bg({ bounds.size.x + 40.f, bounds.size.y + 20.f });
	bg.setOrigin({ bg.getSize().x / 2.f, 0.f});
	bg.setPosition({ x, y - 5.f });
	bg.setFillColor(sf::Color(20, 20, 20, 200));
	bg.setOutlineThickness(2.f);
	bg.setOutlineColor(isPrepPhase ? sf::Color(0,120,0,200) : sf::Color(150,0,0,200));
...
void UIManager::draw(sf::RenderWindow& window, const Player& player, const sf::View& view, float prepTimer, float waveTimer, int currWave, bool isPrepPhase) {
    if (!m_ready) return;
    ...
    drawTooltip(window);
    drawTimerUI(window, prepTimer, waveTimer, currWave, isPrepPhase);
