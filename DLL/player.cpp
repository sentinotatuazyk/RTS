...
void Player::changeTownHallStatus(bool built)
{
    m_TownHallBuilt = built;
    if (!built) m_TownHallDestroyed = true;
}
    ...
    if (const auto* mousePressed = event.getIf<sf::Event::MouseButtonPressed>()) {
        if (mousePressed->button == sf::Mouse::Button::Left) {
            ...
            if (m_placingBuilding) {
                ...
                auto cost = getBuildingCost(m_placeType);
				spendResource(ResourceType::Gold, cost.gold);
				spendResource(ResourceType::Wood, cost.wood);
				spendResource(ResourceType::Rock, cost.rock);
