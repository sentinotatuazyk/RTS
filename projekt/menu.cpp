...
Menu::Menu(sf::RenderWindow& window) : m_window(window), m_title(m_font), m_start(m_font), m_exit(m_font) {
    ...
    centerTextX(m_exit, topY + spacing * 3.f);
...
MenuAction Menu::handleEvent(const sf::Event& event) {
    if (const auto* keyPressed = event.getIf<sf::Event::KeyPressed>()) {
        ...
        if (keyPressed->scancode == sf::Keyboard::Scancode::Up) {
            ...
            if (m_selected == 1) return MenuAction::ExitGame;
        }
    }
...
void Menu::updateSelection() {
    ...
    m_exit.setFillColor(m_selected == 1 ? sf::Color::Yellow : sf::Color::White);
}